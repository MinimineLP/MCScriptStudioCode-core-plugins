...
CodeMirror.defineSimpleMode("mcscript", {
  ...
  comment2: [
    ...
    //{regex: /(@(author|package|project|since|version|file|repository|author_url|see))( [a-zA-Z0-9, \\\/\:\{\}\[\]\(\)@]+)/, token: ["comment2_annountion","comment2_annountion_value"], next: "comment2"},
    {regex: new RegExp(`(@(${docTags.join("|")}))`), token: ["comment2_annountion"]},

    {regex: /\<(.+)\>/, token: "comment2_htmltag"},
    {regex: /[\{\[\(]/, indent: true},
    {regex: /[\}\]\)]/, dedent: true},
